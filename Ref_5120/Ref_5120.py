...
while url_index < len(url_list):
    try:
        ...
        first_soup = check_step(lambda: get_soup(url), "Failed to get soup from initial URL")
        current_link_tag = check_step(lambda: first_soup.find("a",string="Current Issue"), "Failed to find current link tag")
        current_link_href = check_step(lambda: current_link_tag["href"], "Failed to find 'href' from current anchor tag")
        current_link = current_link_href.replace("./browse",f"{base_url}browse")
        current_soup = check_step(lambda: get_soup(current_link), "Failed to get soup from current URL")
        print(f"🔗 Current URL: [{current_link}]")

        try:
            Source_title = first_soup.find("title").get_text(strip=True)
        except Exception as error:
            Source_title = None
        print(f"✅ Source_title: {Source_title}")

        try:
            main_text = current_soup.find("div",class_="yw_box").h2.get_text(strip=True)
        ...
        print(f"✅ Volume: {Volume}, Issue: {Issue}, Year: {Year}, Month: {Month}")

        try:
            issn_tag = first_soup.find(lambda tag: tag.name == "li" and re.search(r"ISSN",tag.get_text(strip=True)))
            issn_text = issn_tag.get_text(strip=True)
            issn_match = re.search(r"ISSN:(\d{4}-\d{3}[\dX])", issn_text, re.IGNORECASE)
        ...
        while article_index < len(All_articles):
            ...
            Art_index=All_articles[article_index].find_parent("div",class_="yw_text").find_parent("div",class_="yw_text").find_parent("div",class_="yw_text")
            current_step = None
            try:
                ...
                try:
                    article_type_tag = Article_details.find(
                        lambda tag: (
                                tag.name == "div"
                                and re.search(r"Type of Article", tag.get_text(strip=True))
                                and all(cls in (tag.get("class") or []) for cls in ["yw_text_small", "abstract"])
                        )
                    )
                    Article_type = article_type_tag.a.get_text()
                except Exception:
                    Article_type = None
                ...
                pdf_link = pdf_link_tag["href"].replace("./",f"{base_url}")

                if article_check==0:
                    print("✅ "+get_ordinal_suffix(Article_count) + " article details have been scraped")
                    print(f"🔗 Article URL: [{Article_link}]")
                    print(f"🔗 PDF URL: [{pdf_link}]")
