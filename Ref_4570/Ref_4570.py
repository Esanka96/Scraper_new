...
while url_index < len(url_list):
    try:
        ...
        try:
            ...
            current_tag = first_soup.find("div", attrs={"id":"toc"}).find("div",class_="item")
            current_link = current_tag.find_all("h3",class_="issue")[-1].find("a")["href"]
        ...
        try:
            Source_title = first_soup.find("div",attrs={"id":"pagetitle"}).h6.get_text(strip=True)
        except Exception as error:
            Source_title = None
        ...
        try:
            issn_tag = current_soup.find("strong",string=re.compile(r"ISSN"))
            issn_text = issn_tag.get_text(strip=True)
            ISSN = issn_text.split()[1]
        except Exception as error:
            ISSN = None
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                try:
                    identifier_tag = Article_details.find("p",class_="citation")
                    identifier_text = identifier_tag.get_text(strip=True)
                    Identifier = re.search(r"Article (\d+). ",identifier_text).group(1)
                except Exception:
                    Identifier = None
