...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        base_url = "https://cpsyjournal.org"

        try:
            first_soup = get_soup(url)
            item_tag = first_soup.find("h1",attrs={"id":"issue-archive"}).find_next_sibling("ul").find("li")
            current_tag = item_tag.find("a")
            current_link = base_url + current_tag["href"]
        ...
        try:
            Source_title = current_soup.find("h1").get_text(strip=True)
        except Exception as error:
            Source_title = None

        try:
            main_text = current_soup.find("h2").get_text(strip=True)
        ...
        try:
            issn_tag = current_soup.find("li",string=re.compile(r"ISSN"))
        ...
        try:
            All_articles = current_soup.find_all("li",attrs={"data-sentry-component":"ArticleCardListLayout"})
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                article_link_tag = Art_index.find("div",attrs={"data-sentry-component":"Title"})
                Article_link = base_url+article_link_tag.a["href"]

                current_step = "article title"
                article_title_tag = Art_index.find("div",attrs={"data-sentry-component":"Title"})
                ...
                try:
                    article_type_tag = Article_details.find("h2")
                    Article_type = article_type_tag.get_text(strip=True)
                except Exception:
                    Article_type = None

                try:
                    abstract_tag = Article_details.find("h2",attrs={"id":"abstract"}).find_next_sibling("div")
                    Abstract = abstract_tag.get_text(strip=True)
                except Exception:
                    Abstract = None

                try:
                    page_range_tag = Article_details.find("div",attrs={"data-sentry-component":"Pages"})
                    Page_range = re.split(r":\s+",page_range_tag.get_text(strip=True))[-1]
                except Exception:
                    Page_range = None

                try:
                    doi_tag = Article_details.find("a",attrs={"data-sentry-source-file":"doi.tsx"})
                    DOI = re.split(r"DOI:?\s+",doi_tag.get_text(strip=True))[-1]
                except Exception:
                    DOI = None

                current_step = "pdf link"
                pdf_link_tag = Art_index.find("a", href=lambda text:text and ".pdf" in text)
