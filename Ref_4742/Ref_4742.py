...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        base_url = "https://ijm.tums.ac.ir/"

        try:
            first_soup = get_soup(url)
            current_link_tag = first_soup.find("a",string=re.compile(r"Current"))
            current_link = current_link_tag["href"]
            current_soup = get_soup(current_link)
            print(f"🔗 Current URL: [{current_link}]")

            current_id = current_link.split("/")[-1]

            russian_soup = get_soup(f"https://permmedjournal.ru/PMJ/user/setLocale/ru_RU?source=%2FPMJ%2Fissue%2Fview%2F{current_id}")

            languageList = [current_soup, russian_soup]
        ...
        try:
            Source_title = current_soup.find("div",attrs={"id":"headerTitle"}).get_text(strip=True)
        except Exception as error:
            Source_title = None

        try:
            main_text = current_soup.find("h1").get_text(strip=True)
        ...
        try:
            issn_tag = current_soup.find("span",string=re.compile(r"ISSN"))
            issn_text = issn_tag.get_text(strip=True)
            ISSN = re.search(r"ISSN\s*(\d+-\d+)",issn_text).group(1)
        except Exception as error:
            ISSN = None

        try:
            All_articles = current_soup.find_all("div",class_="issueArticlesData")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                article_link_tag = Art_index.find("h3",class_="articleTitle")
                Article_link = article_link_tag.a["href"]

                current_step = "article title"
                article_title_tag = Art_index.find("h3",class_="articleTitle")
                ...
                try:
                    article_type_tag = Article_details.find("b",string=re.compile(r"Section:?"))
                    pre_article_type_tag = article_type_tag.find_next_sibling()
                    Article_type = pre_article_type_tag.get_text(strip=True)
                except Exception:
                    Article_type = None

                try:
                    abstract_tag = Article_details.find("h2",string="Abstract")
                    pre_abstract_tag = abstract_tag.find_next_sibling("div")
                    Abstract = pre_abstract_tag.get_text()
                except Exception:
                    Abstract = None

                try:
                    page_range_tag = Article_details.find("b",string=re.compile(r"Pages:"))
                    pre_page_range_tag = page_range_tag.find_next_sibling(string=True)
                    Page_range = pre_page_range_tag.get_text(strip=True)
                except Exception:
                    Page_range = None

                try:
                    doi_tag = Article_details.find("b",string=re.compile(r"DOI:?"))
                    pre_doi_tag = doi_tag.find_next_sibling()
                    DOI = pre_doi_tag.get_text(strip=True).split("org/")[-1]
                except Exception:
                    DOI = None

                current_step = "pdf link"
                pdf_link_tag = Article_details.find("div", attrs={"id":"articleFullText"}).find("a")
                pdf_link = pdf_link_tag["href"].replace("view","download")
